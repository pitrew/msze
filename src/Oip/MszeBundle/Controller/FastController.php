...
class FastController extends Controller
{
    public function churchesAction() {
        ...
        $crepo = $em->getRepository('OipMszeBundle:Church');
        
        $churches = $crepo->findAll();
        ...
        for ($x = 0; $x < sizeof($churches); $x++)
        {
            ...
            $result[$x][] = $churches[$x]->getId();
            $result[$x][] = $churches[$x]->getLatitude();
            $result[$x][] = $churches[$x]->getLongitude();
        }
